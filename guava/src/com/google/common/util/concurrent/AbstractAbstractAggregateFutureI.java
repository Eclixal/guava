...
import static com.google.common.util.concurrent.AbstractAbstractAggregateFutureI.ReleaseResourcesReason.ALL_INPUT_FUTURES_PROCESSED;
 ...
 */
@GwtCompatible
abstract class AbstractAbstractAggregateFutureI<InputT, OutputT> extends AbstractAggregateFutureStateI<OutputT> {
  private static final Logger logger = Logger.getLogger(AbstractAbstractAggregateFutureI.class.getName());
   ...
   * of {@link CombinedFutureAbstractAbstractI}, the user-supplied callback usually has its own references to inputs.
   */
  /*
   * In certain circumstances, this field might theoretically not be visible to an afterDone() call
   * triggered by cancel(). For details, see the comments on the fields of TimeoutFuture.
   */
  private @Nullable ImmutableCollection<? extends IListenableFuture<? extends InputT>> futures;

  private final boolean allMustSucceed;
  private final boolean collectsValues;

  AbstractAbstractAggregateFutureI(
      ImmutableCollection<? extends IListenableFuture<? extends InputT>> futures,
  ...
  final void init() {
    ...
    if (allMustSucceed) {
      ...
      int i = 0;
      for (final IListenableFuture<? extends InputT> future : futures) {
        final int index = i++;
        future.addListener(
            new Runnable() {
              @Override
      ...
      for (IListenableFuture<? extends InputT> future : futures) {
