...
import static com.google.common.util.concurrent.IService.State.FAILED;
import static com.google.common.util.concurrent.IService.State.NEW;
import static com.google.common.util.concurrent.IService.State.RUNNING;
import static com.google.common.util.concurrent.IService.State.STARTING;
import static com.google.common.util.concurrent.IService.State.STOPPING;
import static com.google.common.util.concurrent.IService.State.TERMINATED;
...
/**
 * A manager for monitoring and controlling a set of {@linkplain IService services}. This class
 * provides methods for {@linkplain #startAsync() starting}, {@linkplain #stopAsync() stopping} and
 * {@linkplain #servicesByState inspecting} a collection of {@linkplain IService services}.
 ...
 */
@GwtIncompatible
public final class IServiceManager implements IServiceManagerBridge {
  private static final Logger logger = Logger.getLogger(IServiceManager.class.getName());
   ...
   * IService service} starts, or terminates), should attach {@linkplain IService.Listener service
  ...
  public abstract static class Listener {
     ...
     *
     * @param IService The service that failed.
     */
   ...
   * IServiceManager} constructor without having to close over the partially constructed {@link
   * IServiceManager} instance (i.e. avoid leaking a pointer to {@code this}).
   */
  private final ServiceManagerState state;

  private final ImmutableList<IService> IServices;
   ...
   */
  public IServiceManager(Iterable<? extends IService> services) {
    ImmutableList<IService> copy = ImmutableList.copyOf(services);
    if (copy.isEmpty()) {
      ...
      copy = ImmutableList.<IService>of(new NoOpIService());
    }
    this.state = new ServiceManagerState(copy);
    this.IServices = copy;
    WeakReference<ServiceManagerState> stateReference = new WeakReference<>(state);
    for (IService IService : copy) {
      IService.addListener(new ServiceListener(IService, stateReference), directExecutor());
      // We check the state after adding the listener as a way to ensure that our listener was added
      // to a NEW service.
      checkArgument(IService.state() == NEW, "Can only manage NEW services, %s", IService);
    }
   ...
   * listeners are added before any of the managed services are {@linkplain IService#startAsync
   * started}.
   *
  ...
  /**
   * Initiates service {@linkplain IService#startAsync startup} on all the services being managed. It
  ...
  public IServiceManager startAsync() {
    for (IService IService : IServices) {
      State state = IService.state();
      checkState(state == NEW, "Service %s is %s, cannot start it.", IService, state);
    }
    for (IService IService : IServices) {
      try {
        state.tryStartTiming(IService);
        IService.startAsync();
        ...
        logger.log(Level.WARNING, "Unable to start Service " + IService, e);
      }
    }
    return this;
  }
  ...
  /**
   * Waits for the {@link IServiceManager} to become {@linkplain #isHealthy() healthy}. The manager
  ...
  /**
   * Waits for the {@link IServiceManager} to become {@linkplain #isHealthy() healthy} for no more
  ...
  /**
   * Initiates service {@linkplain IService#stopAsync shutdown} if necessary on all the services
   * being managed.
   *
   * @return this
   */
  @CanIgnoreReturnValue
  public IServiceManager stopAsync() {
    for (IService IService : IServices) {
      IService.stopAsync();
    }
    return this;
   ...
   * services will either be {@linkplain IService.State#TERMINATED terminated} or {@linkplain
   * IService.State#FAILED failed}.
   */
   ...
   * this method returns all services will either be {@linkplain IService.State#TERMINATED
   * terminated} or {@linkplain IService.State#FAILED failed}.
   *
   * @param timeout the maximum time to wait
   ...
   */
  @Override
  public ImmutableSetMultimap<State, IService> servicesByState() {
   ...
   */
  public ImmutableMap<IService, Long> startupTimes() {
    return state.startupTimes();
  }

  @Override
  public String toString() {
    return MoreObjects.toStringHelper(IServiceManager.class)
        .add("services", Collections2.filter(IServices, not(instanceOf(NoOpIService.class))))
        .toString();
  }

  /**
   * An encapsulation of all the mutable state of the {@link IServiceManager} that needs to be
  ...
  private static final class ServiceManagerState {
    ...
    @GuardedBy("monitor")
    final SetMultimap<State, IService> servicesByState =
    ...
    @GuardedBy("monitor")
    final Map<IService, Stopwatch> startupTimers = Maps.newIdentityHashMap();
     ...
     */
    ServiceManagerState(ImmutableCollection<IService> IServices) {
      this.numberOfServices = IServices.size();
      servicesByState.putAll(NEW, IServices);
    }
     ...
     * IService#startAsync()}.
     */
    void tryStartTiming(IService IService) {
      monitor.enter();
      try {
        Stopwatch stopwatch = startupTimers.get(IService);
        if (stopwatch == null) {
          startupTimers.put(IService, Stopwatch.createStarted());
        }
      } finally {
        monitor.leave();
      }
    ...
    void markReady() {
      ...
      try {
        if (!transitioned) {
          ...
          List<IService> servicesInBadStates = Lists.newArrayList();
          for (IService IService : servicesByState().values()) {
            if (IService.state() != NEW) {
              servicesInBadStates.add(IService);
            }
          }
    ...
    ImmutableSetMultimap<State, IService> servicesByState() {
      ImmutableSetMultimap.Builder<State, IService> builder = ImmutableSetMultimap.builder();
      monitor.enter();
      try {
        for (Entry<State, IService> entry : servicesByState.entries()) {
          if (!(entry.getValue() instanceof NoOpIService)) {
            builder.put(entry);
          }
        }
      } finally {
    ...
    ImmutableMap<IService, Long> startupTimes() {
      List<Entry<IService, Long>> loadTimes;
      monitor.enter();
      try {
        ...
        for (Entry<IService, Stopwatch> entry : startupTimers.entrySet()) {
          IService IService = entry.getKey();
          Stopwatch stopWatch = entry.getValue();
          if (!stopWatch.isRunning() && !(IService instanceof NoOpIService)) {
            loadTimes.add(Maps.immutableEntry(IService, stopWatch.elapsed(MILLISECONDS)));
          }
        }
      } finally {
        monitor.leave();
      ...
      Collections.sort(
          ...
          Ordering.natural()
              .onResultOf(
                  new Function<Entry<IService, Long>, Long>() {
                    @Override
                    public Long apply(Entry<IService, Long> input) {
     ...
     * </ol>
     */
    void transitionService(final IService IService, State from, State to) {
      checkNotNull(IService);
      ...
      try {
        ...
        // Update state.
        checkState(
            servicesByState.remove(from, IService),
            "Service %s not at the expected location in the state map %s",
                IService,
            from);
        checkState(
            servicesByState.put(to, IService),
        ...
        if (stopwatch == null) {
          ...
          startupTimers.put(IService, stopwatch);
        }
        if (to.compareTo(RUNNING) >= 0 && stopwatch.isRunning()) {
          // N.B. if we miss the STARTING event then we may never record a startup time.
          stopwatch.stop();
          if (!(IService instanceof NoOpIService)) {
            logger.log(Level.FINE, "Started {0} in {1}.", new Object[] {IService, stopwatch});
          }
        }
        // Queue our listeners
    ...
    void enqueueFailedEvent(final IService IService) {
      listeners.enqueue(
          new ListenerCallQueue.Event<Listener>() {
            @Override
            public void call(Listener listener) {
              listener.failure(IService);
            }

            @Override
            public String toString() {
              return "failed({service=" + IService + "})";
            }
          });
    }
    ...
    void checkHealthy() {
      if (states.count(RUNNING) != numberOfServices) {
        ...
        for (IService IService : servicesByState.get(State.FAILED)) {
          exception.addSuppressed(new FailedService(IService));
        }
        throw exception;
      }
    }
  ...
  /**
   * A {@link IService} that wraps another service and times how long it takes for it to start and
   * also calls the {@link ServiceManagerState#transitionService(IService, State, State)}, to record
   * the state transitions.
   */
  private static final class ServiceListener extends IService.Listener {
    ...
    ServiceListener(IService IService, WeakReference<ServiceManagerState> state) {
    ...
    public void starting() {
      ...
      if (state != null) {
        state.transitionService(IService, NEW, STARTING);
        if (!(IService instanceof NoOpIService)) {
          logger.log(Level.FINE, "Starting {0}.", IService);
        }
      }
    }
    ...
    public void running() {
      ServiceManagerState state = this.state.get();
      if (state != null) {
        state.transitionService(IService, STARTING, RUNNING);
      }
    }
    ...
    public void stopping(State from) {
      ServiceManagerState state = this.state.get();
      if (state != null) {
        state.transitionService(IService, from, STOPPING);
      }
    }
      ...
      if (state != null) {
        if (!(IService instanceof NoOpIService)) {
          logger.log(
              ...
              new Object[] {IService, from});
        }
        state.transitionService(IService, from, TERMINATED);
      }
    }
    ...
    public void failed(State from, Throwable failure) {
      ...
      if (state != null) {
        ...
        boolean log = !(IService instanceof NoOpIService);
        /*
        ...
        if (log) {
          logger.log(
              Level.SEVERE,
              "Service " + IService + " has failed in the " + from + " state.",
              failure);
        }
        state.transitionService(IService, from, FAILED);
      }
    }
  }

  /**
   * A {@link IService} instance that does nothing. This is only useful as a placeholder to ensure
   * that the {@link IServiceManager} functions properly even when it is managing no services.
   *
   ...
   * logging statements.
   */
  private static final class NoOpIService extends AbstractIService {
    @Override
  ...
  private static final class FailedService extends Throwable {
    FailedService(IService IService) {
      super(
          IService.toString(),
          IService.failureCause(),
