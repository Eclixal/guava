...
/**
 * A {@link IListenableFuture} that supports fluent chains of operations. For example:
 *
 * <pre>{@code
 ...
 * own subclass of {@link IListenableFuture}, complete with a method like {@link #from} to adapt an
 * existing {@code ListenableFuture}, implemented atop a {@link ForwardingIListenableFuture} that
...
public abstract class FluentFutureI<V> extends GwtFluentFutureCatchingSpecializationI<V> {

  /**
   * A less abstract subclass of AbstractFuture. This can be used to optimize setFuture by ensuring
   * that {@link #get} calls exactly the implementation of {@link AbstractFutureI#get}.
   */
  abstract static class TrustedFutureI<V> extends FluentFutureI<V>
      implements AbstractFutureI.Trusted<V> {
   ...
   */
  public static <V> FluentFutureI<V> from(IListenableFuture<V> future) {
    return future instanceof FluentFutureI
        ? (FluentFutureI<V>) future
        : new ForwardingFluentFutureI<V>(future);
  }
   ...
   */
  @Deprecated
  public static <V> FluentFutureI<V> from(FluentFutureI<V> future) {
  ...
  public final <X extends Throwable> FluentFutureI<V> catching(
      Class<X> exceptionType, Function<? super X, ? extends V> fallback, Executor executor) {
    return (FluentFutureI<V>) Futures.catching(this, exceptionType, fallback, executor);
  }

  /**
   * Returns a {@code Future} whose result is taken from this {@code Future} or, if this {@code
   * Future} fails with the given {@code exceptionType}, from the result provided by the {@code
   * fallback}. {@link IAsyncFunction#apply} is not invoked until the primary input has failed, so if
   ...
   *     unrecoverable errors, callers should prefer more specific types, avoiding {@code
   *     Throwable.class} in particular.
   * @param fallback the {@link IAsyncFunction} to be called if the input fails with the expected
   *     exception type. The function's argument is the input's exception. "The input's exception"
   ...
   */
  @Partially.GwtIncompatible("AVAILABLE but requires exceptionType to be Throwable.class")
  public final <X extends Throwable> FluentFutureI<V> catchingAsync(
          Class<X> exceptionType, IAsyncFunction<? super X, ? extends V> fallback, Executor executor) {
    return (FluentFutureI<V>) Futures.catchingAsync(this, exceptionType, fallback, executor);
  }
  ...
  public final FluentFutureI<V> withTimeout(
  ...
  public final FluentFutureI<V> withTimeout(
    ...
    return (FluentFutureI<V>) Futures.withTimeout(this, timeout, unit, scheduledExecutor);
  }
   ...
   */
  public final <T> FluentFutureI<T> transformAsync(
          IAsyncFunction<? super V, T> function, Executor executor) {
    return (FluentFutureI<T>) Futures.transformAsync(this, function, executor);
  }
   ...
   */
  public final <T> FluentFutureI<T> transform(Function<? super V, T> function, Executor executor) {
    return (FluentFutureI<T>) Futures.transform(this, function, executor);
  }
   ...
   */
  public final void addCallback(IFutureCallback<? super V> callback, Executor executor) {
