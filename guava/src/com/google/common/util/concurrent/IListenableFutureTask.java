...
/**
 * A {@link FutureTask} that also implements the {@link IListenableFuture} interface. Unlike {@code
 ...
 * {@code ExecutorService}. Most users should call {@link IListeningExecutorService#submit(Callable)
 ...
 */
@GwtIncompatible
public class IListenableFutureTask<V> extends FutureTask<V> implements IListenableFuture<V> {
   ...
   * @since 10.0
   */
  public static <V> IListenableFutureTask<V> create(Callable<V> callable) {
    return new IListenableFutureTask<V>(callable);
  }
   ...
   * @since 10.0
   */
  public static <V> IListenableFutureTask<V> create(Runnable runnable, @Nullable V result) {
    return new IListenableFutureTask<V>(runnable, result);
  }

  IListenableFutureTask(Callable<V> callable) {
    super(callable);
  }

  IListenableFutureTask(Runnable runnable, @Nullable V result) {
