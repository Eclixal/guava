...
import static com.google.common.util.concurrent.AbstractAbstractAggregateFutureI.ReleaseResourcesReason.OUTPUT_FUTURE_DONE;
...
@GwtCompatible
final class CombinedFutureAbstractAbstractI<V> extends AbstractAbstractAggregateFutureI<Object, V> {
  private CombinedFutureInterruptibleTask<?> task;

  CombinedFutureAbstractAbstractI(
      ImmutableCollection<? extends IListenableFuture<?>> futures,
      boolean allMustSucceed,
      Executor listenerExecutor,
      IAsyncCallable<V> callable) {
  ...
  private abstract class CombinedFutureInterruptibleTask<T> extends InterruptibleTask<T> {
    ...
    final boolean isDone() {
      return CombinedFutureAbstractAbstractI.this.isDone();
    }
    ...
    final void execute() {
      try {
        ...
        CombinedFutureAbstractAbstractI.this.setException(e);
      }
    }
    ...
    final void afterRanInterruptibly(T result, Throwable error) {
       ...
       * pendingToString().)
       */
      CombinedFutureAbstractAbstractI.this.task = null;

      if (error != null) {
        if (error instanceof ExecutionException) {
          CombinedFutureAbstractAbstractI.this.setException(error.getCause());
        } else if (error instanceof CancellationException) {
          cancel(false);
        } else {
          CombinedFutureAbstractAbstractI.this.setException(error);
        }
      } else {
        setValue(result);
      }
  ...
  private final class AsyncCallableInterruptibleTask
      extends CombinedFutureInterruptibleTask<IListenableFuture<V>> {
    private final IAsyncCallable<V> callable;

    AsyncCallableInterruptibleTask(IAsyncCallable<V> callable, Executor listenerExecutor) {
    ...
    IListenableFuture<V> runInterruptibly() throws Exception {
      IListenableFuture<V> result = callable.call();
      return checkNotNull(
          result,
    ...
    @Override
    void setValue(IListenableFuture<V> value) {
      CombinedFutureAbstractAbstractI.this.setFuture(value);
    }
  ...
  private final class CallableInterruptibleTask extends CombinedFutureInterruptibleTask<V> {
    ...
    void setValue(V value) {
      CombinedFutureAbstractAbstractI.this.set(value);
    }
