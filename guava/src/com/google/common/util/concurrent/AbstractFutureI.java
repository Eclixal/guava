...
/**
 * An abstract implementation of {@link IListenableFuture}, intended for advanced users only. More
 * common ways to create a {@code ListenableFuture} include instantiating a {@link SettableFutureI},
 * submitting a task to a {@link IListeningExecutorService}, and deriving a {@code Future} from an
 * existing one, typically using methods like {@link Futures#transform(IListenableFuture,
 * com.google.common.base.Function, java.util.concurrent.Executor) Futures.transform} and {@link
 * Futures#catching(IListenableFuture, Class, com.google.common.base.Function,
 ...
 * #setFuture(IListenableFuture)} and {@link #setException(Throwable)}. Subclasses may also override
...
public abstract class AbstractFutureI<V> extends InternalFutureFailureAccess
   ...
   * that {@link #get} calls exactly the implementation of {@link AbstractFutureI#get}.
   */
  abstract static class TrustedFutureI<V> extends AbstractFutureI<V> implements Trusted<V> {
      ...
      try {
        helper =
            new SafeAtomicHelper(
                ...
                newUpdater(AbstractFutureI.class, Waiter.class, "waiters"),
                newUpdater(AbstractFutureI.class, Listener.class, "listeners"),
                newUpdater(AbstractFutureI.class, Object.class, "value"));
  ...
  private static final class SetFuture<V> implements Runnable {
    final AbstractFutureI<V> owner;
    final IListenableFuture<? extends V> future;

    SetFuture(AbstractFutureI<V> owner, IListenableFuture<? extends V> future) {
      this.owner = owner;
      this.future = future;
    }
   ...
   * {@inheritDoc}
   *
   * <p>The default {@link AbstractFutureI} implementation throws {@code InterruptedException} if the
  ...
  public boolean cancel(boolean mayInterruptIfRunning) {
    ...
    if (localValue == null | localValue instanceof SetFuture) {
      ...
      AbstractFutureI<?> abstractFuture = this;
      while (true) {
        if (ATOMIC_HELPER.casValue(abstractFuture, localValue, valueToSet)) {
          ...
          if (localValue instanceof SetFuture) {
            ...
            IListenableFuture<?> futureToPropagateTo = ((SetFuture) localValue).future;
            if (futureToPropagateTo instanceof Trusted) {
              ...
              AbstractFutureI<?> trusted = (AbstractFutureI<?>) futureToPropagateTo;
   ...
   */
  private static Object getFutureValue(IListenableFuture<?> future) {
    if (future instanceof Trusted) {
      ...
      Object v = ((AbstractFutureI<?>) future).value;
  ...
  private static void complete(AbstractFutureI<?> future) {
    Listener next = null;
    outer:
    while (true) {
  ...
  private abstract static class AtomicHelper {
    ...
    abstract boolean casWaiters(AbstractFutureI<?> future, Waiter expect, Waiter update);

    /** Performs a CAS operation on the {@link #listeners} field. */
    abstract boolean casListeners(AbstractFutureI<?> future, Listener expect, Listener update);

    /** Performs a CAS operation on the {@link #value} field. */
    abstract boolean casValue(AbstractFutureI<?> future, Object expect, Object update);
  }
  ...
  private static final class UnsafeAtomicHelper extends AtomicHelper {
    ...
    static {
      ...
      try {
        Class<?> abstractFuture = AbstractFutureI.class;
    ...
    /** Performs a CAS operation on the {@link #waiters} field. */
    @Override
    boolean casWaiters(AbstractFutureI<?> future, Waiter expect, Waiter update) {
      return UNSAFE.compareAndSwapObject(future, WAITERS_OFFSET, expect, update);
    }

    /** Performs a CAS operation on the {@link #listeners} field. */
    @Override
    boolean casListeners(AbstractFutureI<?> future, Listener expect, Listener update) {
      return UNSAFE.compareAndSwapObject(future, LISTENERS_OFFSET, expect, update);
    }

    /** Performs a CAS operation on the {@link #value} field. */
    @Override
    boolean casValue(AbstractFutureI<?> future, Object expect, Object update) {
  ...
  private static final class SafeAtomicHelper extends AtomicHelper {
    ...
    final AtomicReferenceFieldUpdater<AbstractFutureI, Waiter> waitersUpdater;
    final AtomicReferenceFieldUpdater<AbstractFutureI, Listener> listenersUpdater;
    final AtomicReferenceFieldUpdater<AbstractFutureI, Object> valueUpdater;

    SafeAtomicHelper(
        AtomicReferenceFieldUpdater<Waiter, Thread> waiterThreadUpdater,
        AtomicReferenceFieldUpdater<Waiter, Waiter> waiterNextUpdater,
        AtomicReferenceFieldUpdater<AbstractFutureI, Waiter> waitersUpdater,
        AtomicReferenceFieldUpdater<AbstractFutureI, Listener> listenersUpdater,
        AtomicReferenceFieldUpdater<AbstractFutureI, Object> valueUpdater) {
