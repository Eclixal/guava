...
/**
 * A {@link ScheduledExecutorService} that returns {@link IListenableFuture} instances from its
 ...
 */
@GwtIncompatible
public interface IListeningScheduledExecutorService
    extends ScheduledExecutorService, IListeningExecutorService {

  /** @since 15.0 (previously returned ScheduledFuture) */
  @Override
  IListenableScheduledFuture<?> schedule(Runnable command, long delay, TimeUnit unit);
   ...
   * @since 29.0
   */
  default IListenableScheduledFuture<?> schedule(Runnable command, Duration delay) {
    return schedule(command, toNanosSaturated(delay), TimeUnit.NANOSECONDS);
  }

  /** @since 15.0 (previously returned ScheduledFuture) */
  @Override
  <V> IListenableScheduledFuture<V> schedule(Callable<V> callable, long delay, TimeUnit unit);
   ...
   * @since 29.0
   */
  default <V> IListenableScheduledFuture<V> schedule(Callable<V> callable, Duration delay) {
    return schedule(callable, toNanosSaturated(delay), TimeUnit.NANOSECONDS);
  }

  /** @since 15.0 (previously returned ScheduledFuture) */
  @Override
  IListenableScheduledFuture<?> scheduleAtFixedRate(
   ...
   * @since 29.0
   */
  default IListenableScheduledFuture<?> scheduleAtFixedRate(
  ...
  /** @since 15.0 (previously returned ScheduledFuture) */
  @Override
  IListenableScheduledFuture<?> scheduleWithFixedDelay(
   ...
   * @since 29.0
   */
  default IListenableScheduledFuture<?> scheduleWithFixedDelay(
