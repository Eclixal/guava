 ...
 */
@GwtCompatible
final class UncheckedThrowingFutureI<V> extends AbstractFutureI<V> {

  public static <V> IListenableFuture<V> throwingError(Error error) {
    UncheckedThrowingFutureI<V> future = new UncheckedThrowingFutureI<V>();
    future.complete(checkNotNull(error));
    return future;
  }

  public static <V> IListenableFuture<V> throwingRuntimeException(RuntimeException e) {
    UncheckedThrowingFutureI<V> future = new UncheckedThrowingFutureI<V>();
    future.complete(checkNotNull(e));
    return future;
  }

  public static <V> UncheckedThrowingFutureI<V> incomplete() {
    return new UncheckedThrowingFutureI<V>();
  }
