 ...
 */
@GwtIncompatible
public abstract class AbstractScheduledIService implements IService {
  private static final Logger logger = Logger.getLogger(AbstractScheduledIService.class.getName());

  /**
   * A scheduler defines the policy for how the {@link AbstractScheduledIService} should run its
   * task.
   *
  ...
  public abstract static class Scheduler {
    ...
    public static Scheduler newFixedDelaySchedule(
      ...
      return new Scheduler() {
        ...
        public Future<?> schedule(
                AbstractIService service, ScheduledExecutorService executor, Runnable task) {
    ...
    abstract Future<?> schedule(
            AbstractIService service, ScheduledExecutorService executor, Runnable runnable);

    private Scheduler() {}
  }

  /* use AbstractService for state management */
  private final AbstractIService delegate = new IServiceDelegate();

  @WeakOuter
  private final class IServiceDelegate extends AbstractIService {
    ...
    class Task implements Runnable {
      ...
      public void run() {
        ...
        try {
          ...
          AbstractScheduledIService.this.runOneIteration();
        } catch (Throwable t) {
          try {
            shutDown();
    ...
    public String toString() {
      return AbstractScheduledIService.this.toString();
    }
  }

  /** Constructor for use by subclasses. */
  protected AbstractScheduledIService() {}

  /**
   * Run one iteration of the scheduled task. If any invocation of this method throws an exception,
   * the service will transition to the {@link IService.State#FAILED} state and this method will no
   * longer be called.
   */
   ...
   * {@linkplain IService.State#TERMINATED terminates} or {@linkplain IService.State#TERMINATED
  ...
  /**
   * Returns the name of this service. {@link AbstractScheduledIService} may include the name in
   * debugging output.
   *
   * @since 14.0
   */
  ...
  public final void awaitRunning(Duration timeout) throws TimeoutException {
    IService.super.awaitRunning(timeout);
  }
  ...
  public final void awaitTerminated(Duration timeout) throws TimeoutException {
    IService.super.awaitTerminated(timeout);
  }
  ...
  /**
   * A {@link Scheduler} that provides a convenient way for the {@link AbstractScheduledIService} to
  ...
  public abstract static class CustomScheduler extends Scheduler {
    ...
    private class ReschedulableCallable extends ForwardingFuture<Void> implements Callable<Void> {
       ...
       * properly.
       */
      private final AbstractIService service;
     ...
     * AbstractScheduledIService#runOneIteration}.
     *
