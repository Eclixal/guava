...
class ImmediateFutureI<V> implements IListenableFuture<V> {
  static final IListenableFuture<?> NULL = new ImmediateFutureI<>(null);

  private static final Logger log = Logger.getLogger(ImmediateFutureI.class.getName());

  private final @Nullable V value;

  ImmediateFutureI(@Nullable V value) {
    this.value = value;
  }
  ...
  static final class ImmediateFailedFutureI<V> extends AbstractFutureI.TrustedFutureI<V> {
    ImmediateFailedFutureI(Throwable thrown) {
      setException(thrown);
    }
  }

  static final class ImmediateCancelledFutureI<V> extends AbstractFutureI.TrustedFutureI<V> {
    ImmediateCancelledFutureI() {
      cancel(false);
    }
  }
}
