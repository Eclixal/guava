 ...
 */
@GwtIncompatible
final class TimeoutFutureI<V> extends FluentFuture.TrustedFutureI<V> {
  static <V> IListenableFuture<V> create(
      IListenableFuture<V> delegate,
      long time,
      TimeUnit unit,
      ScheduledExecutorService scheduledExecutor) {
    TimeoutFutureI<V> result = new TimeoutFutureI<>(delegate);
  ...
  private @Nullable IListenableFuture<V> delegateRef;
  private @Nullable ScheduledFuture<?> timer;

  private TimeoutFutureI(IListenableFuture<V> delegate) {
    this.delegateRef = Preconditions.checkNotNull(delegate);
  }

  /** A runnable that is called when the delegate or the timer completes. */
  private static final class Fire<V> implements Runnable {
    @Nullable TimeoutFutureI<V> timeoutFutureRef;

    Fire(TimeoutFutureI<V> timeoutFuture) {
    ...
    public void run() {
      ...
      // call to this method.
      TimeoutFutureI<V> timeoutFuture = timeoutFutureRef;
      if (timeoutFuture == null) {
        return;
      }
      IListenableFuture<V> delegate = timeoutFuture.delegateRef;
      if (delegate == null) {
        return;
      }
  ...
  protected String pendingToString() {
    IListenableFuture<? extends V> localInputFuture = delegateRef;
